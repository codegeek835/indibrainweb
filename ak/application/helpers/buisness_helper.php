...
function get_dropdown($id)
{
	$ci=& get_instance();
	$ci->load->database(); 
	$query = $ci->db->query("SELECT * FROM `tbl_sub_category` WHERE category_sub_id = '$id'");
	if($query->num_rows() >0 ){
		return true;
	}else{
		return false;
...
function getAllCategory($start,$end)
{
	$ci=& get_instance();
	$ci->load->database(); 
	$query = $ci->db->query("SELECT * FROM `tbl_category` ORDER BY position ASC LIMIT $start,$end");
	return  $query->result();
}
...
function countCategory()
{
	$ci=& get_instance();
	$ci->load->database(); 
	$query = $ci->db->query("SELECT * FROM `tbl_category`");
