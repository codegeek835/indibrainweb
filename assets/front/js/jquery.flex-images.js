...
(function ($) {
	$.fn.flexImages = function (options) {
		...
		return this.each(function () {
			var grid = $(this),
				containers = $(grid).find(o.container),
				items = [],
				t = new Date().getTime(),
				s = window.getComputedStyle
					? getComputedStyle(containers[0], null)
					: containers[0].currentStyle;
			o.margin =
				(parseInt(s.marginLeft) || 0) +
				(parseInt(s.marginRight) || 0) +
				(Math.round(parseFloat(s.borderLeftWidth)) || 0) +
				(Math.round(parseFloat(s.borderRightWidth)) || 0);
	...
	function makeGrid(grid, items, o, noresize) {
		...
		function _helper(lastRow) {
			if (
				(o.maxRows && rows > o.maxRows) ||
				(o.truncate && lastRow && rows > 1)
			)
