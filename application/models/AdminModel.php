...
class AdminModel extends CI_Model 
{
	...
	public function getTotalUser($data){
	 	...
	 	if(isset($data['user_role']) && strlen($data['user_role']) > 0){
			$this->db->where('user_role',$data['user_role']);
		}
		if(isset($data['user_levelNot'])){
			$this->db->where('user_level !=',$data['user_levelNot']);
		}
		if(isset($data['todayDate']) && strlen($data['todayDate']) > 0){
			$this->db->where('date(created_at)',$data['todayDate']);
		}
