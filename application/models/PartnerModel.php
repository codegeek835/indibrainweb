...
class PartnerModel extends CI_Model {
  ...
  public function get_product_by_id($product_id){
	...
	public function getTotalProducts($user_id=0)
	{
	    if($user_id!=0){
			$this->db->where('pro_user_id', $user_id);
		}
	...
	public function getTotalSalesCurrentMonth($all_id)
	{
	    if(count($all_id)>0){
			$this->db->select('SUM(product_price) as price');
			$this->db->where_in('product_id', $all_id);
	...
	public function getAllSecondChatList($data)
	{
		
		if(isset($data['viewType']) && $data['viewType']=='quarterly'){
			$this->db->select('order_details.*,
							   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) >= 1 and  month(created_at) <= 3) as JanTOMarchSales,
							   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) >= 4 and  month(created_at) <= 6) as MayTOJuneSales,
							   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) >= 7 and  month(created_at) <= 9) as JulTOSeptSales,
							   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) >= 10 and  month(created_at) <= 12) as OctTODecSales
							   
							   ');
		}else if(isset($data['viewType']) && $data['viewType']=='yearly'){
			$this->db->select('order_details.*,
							   (SELECT sum(product_price) FROM `order_details` WHERE year(created_at) = '.date('Y',strtotime('NOW')).') as currentYearSale,
							   (SELECT sum(product_price) FROM `order_details` WHERE year(created_at) = '.date('Y',strtotime('- 1 Years')).') as firstYearSale,
							   (SELECT sum(product_price) FROM `order_details` WHERE year(created_at) = '.date('Y',strtotime('- 2 Years')).') as secondYearSale,
							   (SELECT sum(product_price) FROM `order_details` WHERE year(created_at) = '.date('Y',strtotime('- 3 Years')).') as thirdYearSale,
							   (SELECT sum(product_price) FROM `order_details` WHERE year(created_at) = '.date('Y',strtotime('- 4 Years')).') as fourYearSale,
							   (SELECT sum(product_price) FROM `order_details` WHERE year(created_at) = '.date('Y',strtotime('- 5 Years')).') as fiveYearSale,
							   
							   ');
		}else{
		
		$this->db->select('order_details.*,
						   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) = 1) as JanSales,
						   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) = 2) as FebSales,
						   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) = 3) as MarSales,
						   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) = 4) as AprSales,
						   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) = 5) as MaySales,
						   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) = 6) as JunSales,
						   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) = 7) as JulSales,
						   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) = 8) as AugSales,
						   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) = 9) as SepSales,
						   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) = 10) as OctSales,
						   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) = 11) as NovSales,
						   (SELECT sum(product_price) FROM `order_details` WHERE month(created_at) = 12) as DecSales
						   ');
		}
		$this->db->from('order_details');
		$query = $this->db->get();
		$result['data']=$query->result();
		$result['recordCount'] = $query->num_rows();
		return $result;		
	}
